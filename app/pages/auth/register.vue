<template>
  <div>
    <div class="mb-7">
      <h2 class="text-2xl font-bold mb-1.5" style="font-family:'Space Grotesk',sans-serif; color:#ECECF5;">Buat Akun Gratis</h2>
      <p class="text-sm" style="color:rgba(236,236,245,0.45);">Mulai generate video YouTube pertama kamu hari ini.</p>
    </div>
    ...
    <div class="space-y-3 mb-6">
      <button type="button" @click="showDevNotice('Google')"
        ...
        <span class="flex-1 text-center">Daftar dengan Google</span>
      </button>
      ...
      <button type="button" @click="showDevNotice('Facebook')"
        ...
        <span class="flex-1 text-center">Daftar dengan Facebook</span>
      </button>
    </div>
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <span class="text-xs" style="color:rgba(236,236,245,0.3);">atau daftar dengan email</span>
      <div class="flex-1 h-px" style="background:rgba(255,255,255,0.08);"></div>
    </div>
    ...
    <form @submit.prevent="handleRegister" class="space-y-4">
      <!-- Full Name -->
      <div>
        <label class="block text-xs font-semibold mb-2" style="color:rgba(236,236,245,0.6);">Nama Lengkap</label>
        <input v-model="form.name" type="text" required autocomplete="name" placeholder="Nama kamu" class="input-dark" />
      </div>

      <!-- Email -->
      <div>
        <label class="block text-xs font-semibold mb-2" style="color:rgba(236,236,245,0.6);">Email</label>
        <input v-model="form.email" type="email" required autocomplete="email" placeholder="nama@email.com" class="input-dark" />
      </div>

      <!-- Phone -->
      <div>
        <label class="block text-xs font-semibold mb-2" style="color:rgba(236,236,245,0.6);">Nomor HP <span style="color:rgba(236,236,245,0.3);">(opsional)</span></label>
        <div class="relative">
          <div class="absolute left-3.5 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pr-3" style="border-right:1px solid rgba(255,255,255,0.1);">
            <span class="text-sm">🇮🇩</span>
            <span class="text-sm font-medium" style="color:rgba(236,236,245,0.6);">+62</span>
          </div>
          <input v-model="form.phone" type="tel" autocomplete="tel" placeholder="812 3456 7890" class="input-dark pl-24" />
        </div>
      </div>

      <!-- Password -->
      <div>
        <label class="block text-xs font-semibold mb-2" style="color:rgba(236,236,245,0.6);">Password</label>
        <div class="relative">
          <input v-model="form.password" :type="showPwd ? 'text' : 'password'" required autocomplete="new-password" placeholder="Min. 8 karakter" class="input-dark pr-11" />
        ...
        <div class="flex gap-1 mt-2" v-if="form.password">
          <div v-for="i in 4" :key="i" class="h-1 flex-1 rounded-full transition-all"
            :style="`background:${i <= pwdStrength ? pwdColor : 'rgba(255,255,255,0.08)'};`"></div>
        </div>
        <p v-if="form.password" class="text-xs mt-1.5" :style="`color:${pwdColor};`">{{ pwdLabel }}</p>
      </div>

      <!-- Terms -->
      <div class="flex items-start gap-3">
        <input v-model="agreedToTerms" type="checkbox" id="terms" required class="mt-0.5 accent-violet-500 w-4 h-4 flex-shrink-0" />
        <label for="terms" class="text-xs leading-relaxed" style="color:rgba(236,236,245,0.5);">
          Saya menyetujui
          <a href="#" class="font-medium" style="color:#8B5CF6;">Syarat & Ketentuan</a>
          dan
          <a href="#" class="font-medium" style="color:#8B5CF6;">Kebijakan Privasi</a>
          GriyaITNusantara
        </label>
      </div>
      ...
      <button type="submit" :disabled="loading || !agreedToTerms"
        class="btn-primary w-full py-3 text-sm font-semibold mt-1"
...
const pwdColor = computed(() => ['#f87171', '#fbbf24', '#fb923c', '#4ade80'][pwdStrength.value - 1] || '#f87171')
const pwdLabel = computed(() => ['Lemah', 'Cukup', 'Kuat', 'Sangat Kuat'][pwdStrength.value - 1] || 'Lemah')
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    const phone = form.phone ? `+62${form.phone.replace(/^0/, '').replace(/\s/g, '')}` : undefined
    await $fetch('/api/auth/register', {
      method: 'POST',
      body: { email: form.email, password: form.password, name: form.name, phone },
    })
    success.value = 'Akun berhasil dibuat! Silakan masuk.'
    setTimeout(() => navigateTo('/auth/login'), 2000)
  } catch (e: any) {
    error.value = e.data?.message || e.message || 'Pendaftaran gagal, coba lagi'
  } finally {
    loading.value = false
  }
}
